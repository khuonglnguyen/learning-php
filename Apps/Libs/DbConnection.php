...
class Apps_Libs_DbConnection
{
    ...
    public function select(){
        $sql ='select '.$this->queryParams['select'].'from '.$this->tableName.$this->buildCondition($this->queryParams['where']).' '.$this->queryParams['order'];
        $query = $this->query($sql,$this->queryParams['params']);
        return $query->fetchAll(PDO::FETCH_ASSOC);
    }

    public function selectOne(){
        $this->queryParams['order']='limit 1';
        $data = $this->select();
        if ($data) {
            return $data[0];
        }
